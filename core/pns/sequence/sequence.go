...
var (
	_ pns.Index   = (*index)(nil)
	_ pns.Lookup  = (*finder)(nil)
	_ pns.Lookup  = (*asyncFinder)(nil)
	_ pns.Updater = (*updater)(nil)
)

// index just wraps a uint64. implements the pns.Index interface
type index struct {
	index uint64
}
...
// Next requires
func (i *index) Next(last int64, at uint64) pns.Index {
...
// NewFinder constructs an finder (pns.Lookup interface)
func NewFinder(getter storage.Getter, feed *pns.Feed) pns.Lookup {
	return &finder{pns.NewGetter(getter, feed)}
}

// At looks up the version valid at time `at`
// after is a unix time hint of the latest known update
func (f *finder) At(ctx context.Context, at, after int64) (ch swarm.Chunk, current, next pns.Index, err error) {
	for i := uint64(0); ; i++ {
...
func NewAsyncFinder(getter storage.Getter, feed *pns.Feed) pns.Lookup {
	return &asyncFinder{pns.NewGetter(getter, feed)}
}
...
// At looks up the version valid at time `at`
// after is a unix time hint of the latest known update
func (f *asyncFinder) At(ctx context.Context, at, after int64) (ch swarm.Chunk, cur, next pns.Index, err error) {
