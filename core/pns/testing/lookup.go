// package testing provides tests for update  and resolution of time-based pns
package testing
...
func TestFinderBasic(t *testing.T, finderf func(storage.Getter, *pns.Feed) pns.Lookup, updaterf func(putter storage.Putter, signer crypto.Signer, topic []byte) (pns.Updater, error)) {
	...
	t.Run("no update", func(t *testing.T) {
		ch, err := pns.Latest(ctx, finder, 0)
		if err != nil {
			t.Fatal(err)
		}
		if ch != nil {
	...
	t.Run("first update", func(t *testing.T) {
		...
		exp := payload
		ts, payload, err := pns.FromChunk(ch)
		if err != nil {
			t.Fatal(err)
		}
...
func TestFinderFixIntervals(t *testing.T, nextf func() (bool, int64), finderf func(storage.Getter, *pns.Feed) pns.Lookup, updaterf func(putter storage.Putter, signer crypto.Signer, topic []byte) (pns.Updater, error)) {
	var stop bool
	for j := 10; !stop; j += 10 {
...
func TestFinderIntervals(t *testing.T, nextf func() (bool, int64), finderf func(storage.Getter, *pns.Feed) pns.Lookup, updaterf func(putter storage.Putter, signer crypto.Signer, topic []byte) (pns.Updater, error)) {
...
func TestFinderRandomIntervals(t *testing.T, finderf func(storage.Getter, *pns.Feed) pns.Lookup, updaterf func(putter storage.Putter, signer crypto.Signer, topic []byte) (pns.Updater, error)) {
	for j := 0; j < 3; j++ {
