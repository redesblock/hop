...
func testCaseNotAccepted(t *testing.T, recorder *streamtest.Recorder, observer *testObserver, payer, recipient *pseudo.Service, peerID swarm.Address, payerTime, recipientTime int64, recordsLength int, debtAmount, amount *big.Int, expectedError error) {
	...
	records, err := recorder.Records(peerID, "pseudo", "1.0.0", "pseudo")
	if err != nil {
		t.Fatal(err)
	}
...
func testCaseAccepted(t *testing.T, recorder *streamtest.Recorder, observer *testObserver, payer, recipient *pseudo.Service, peerID swarm.Address, payerTime, recipientTime int64, recordsLength, msgLength, recMsgLength int, debtAmount, amount, accepted, totalAmount *big.Int) {
	...
	if receivedAmount.Cmp(accepted) != 0 {
		t.Fatalf("wrong settle amount, got %v, want %v", receivedAmount, accepted)
	}
...
func TestPayment(t *testing.T) {
	...
	recipient := pseudo.New(nil, logger, storeRecipient, observer, big.NewInt(testRefreshRate), big.NewInt(testRefreshRateLight), mockp2p.New())
	...
	payer := pseudo.New(recorder, logger, storePayer, observer2, big.NewInt(testRefreshRate), big.NewInt(testRefreshRateLight), mockp2p.New())
...
func TestTimeLimitedPayment(t *testing.T) {
	...
	// Set time 3 seconds later, attempt settle below time based refreshment rate, expect full amount accepted
	...
	// set time 1 seconds later, attempt settle over the time-based allowed limit, expect partial amount accepted
	...
	// set time to same second as previous case, attempt settle, expect error too soon
	testCaseNotAccepted(t, recorder, observer, payer, recipient, peerID, 10004, 10004, 3, big.NewInt(4*testRefreshRate), big.NewInt(4*testRefreshRate), pseudo.ErrSettlementTooSoon)

	// set time to same second as previous case on recipient, 1 second later on payer, attempt settle, expect sent but failed
...
func TestTimeLimitedPaymentLight(t *testing.T) {
	...
	observer2 := newTestObserver(map[string]*big.Int{}, map[string]*big.Int{peerID.String(): big.NewInt(debt)})
	payer := pseudo.New(recorder, logger, storePayer, observer2, big.NewInt(testRefreshRateLight), big.NewInt(testRefreshRateLight), mockp2p.New())
	payer.SetAccounting(observer2)

	// Set time to 10000, attempt payment based on debt, expect full amount accepted
	testCaseAccepted(t, recorder, observer, payer, recipient, peerID, 10000, 10000, 1, 1, 1, big.NewInt(debt), big.NewInt(debt), big.NewInt(debt), big.NewInt(debt))
	// Set time 3 seconds later, attempt settle below time based light refreshment rate, expect full amount accepted
	...
	// set time 1 seconds later, attempt settle over the time-based light allowed limit, expect partial amount accepted
