...
import (
	...
	pb "github.com/redesblock/hop/core/settle/pseudo/pb"
...
var (
	...
	ErrSettlementTooSoon              = errors.New("settle too soon")
	ErrNoPseudoSettlePeer             = errors.New("settle peer not found")
	ErrDisconnectAllowanceCheckFailed = errors.New("settle allowance below enforced amount")
	ErrTimeOutOfSync                  = errors.New("settle allowance timestamps differ beyond tolerance")
)
...
func New(streamer p2p.Streamer, logger logging.Logger, store storage.StateStorer, accounting settle.Accounting, refreshRate, lightRefreshRate *big.Int, p2pService p2p.Service) *Service {
	return &Service{
		streamer:         streamer,
		logger:           logger,
		metrics:          newMetrics(),
...
func (s *Service) handler(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	...
	if allowance.Cmp(attemptedAmount) < 0 {
		...
		s.logger.Tracef("pseudo accepting reduced payment from peer %v of %d", p.Address, paymentAmount)
	} else {
		s.logger.Tracef("pseudo accepting payment message from peer %v of %d", p.Address, paymentAmount)
	}
...
func (s *Service) Pay(ctx context.Context, peer swarm.Address, amount, checkAllowance *big.Int) (*big.Int, int64, error) {
	...
	s.logger.Tracef("pseudo sending payment message to peer %v of %d", peer, amount)
	w, r := protobuf.NewWriterAndReader(stream)
	...
	if acceptedAmount.Cmp(amount) > 0 {
		err = fmt.Errorf("pseudo peer %v accepted payment larger than expected", peer)
		return nil, 0, err
	}
	...
	if expectedAllowance.Cmp(acceptedAmount) > 0 {
		// disconnect peer
		err = fmt.Errorf("pseudo peer %v accepted lower payment than expected", peer)
		return nil, 0, err
	}
...
// SettlementsSent returns all stored sent settle values for a given type of prefix
func (s *Service) SettlementsSent() (map[string]*big.Int, error) {
	...
	err := s.store.Iterate(SettlementSentPrefix, func(key, val []byte) (stop bool, err error) {
		...
		if _, ok := sent[addr.String()]; !ok {
			...
			if err != nil {
				return false, fmt.Errorf("get peer %s settle balance: %w", addr.String(), err)
			}
...
// SettlementsReceived returns all stored received settle values for a given type of prefix
