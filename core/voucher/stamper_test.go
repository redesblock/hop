...
func TestStamperStamping(t *testing.T) {
	...
	createStamp := func(t *testing.T, stamper voucher.Stamper) (swarm.Address, *voucher.Stamp) {
		t.Helper()
		h := make([]byte, 32)
	...
	t.Run("bucket mismatch", func(t *testing.T) {
		...
		a := chunkAddr.Bytes()
		a[0] ^= 0xff
		if err := stamp.Valid(swarm.NewAddress(a), owner, 12, 8, true); !errors.Is(err, voucher.ErrBucketMismatch) {
