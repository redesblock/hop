...
import (
	...
	postagestoreMock "github.com/redesblock/hop/core/voucher/batchstore/mock"
	postageMock "github.com/redesblock/hop/core/voucher/mock"
...
func TestCreateBatch(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		expectedCallData, err := vouchercontract.PostageStampABI.Pack("createBatch", owner, initialBalance, depth, vouchercontract.BucketDepth, common.Hash{}, false)
		if err != nil {
			t.Fatal(err)
		}
...
func newCreateEvent(postageContractAddress common.Address, batchId common.Hash) *types.Log {
	b, err := vouchercontract.PostageStampABI.Events["BatchCreated"].Inputs.NonIndexed().Pack(
		big.NewInt(0),
		big.NewInt(0),
		common.Address{},
		uint8(1),
...
func TestTopUpBatch(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		postageMock := postageMock.New(postageMock.WithIssuer(voucher.NewStampIssuer(
			"label",
			"keyID",
			batch.ID,
			batch.Value,
		...
		expectedCallData, err := vouchercontract.PostageStampABI.Pack("topUp", common.BytesToHash(batch.ID), topupBalance)
		if err != nil {
			t.Fatal(err)
		}
...
func newTopUpEvent(postageContractAddress common.Address, batch *voucher.Batch) *types.Log {
	b, err := vouchercontract.PostageStampABI.Events["BatchTopUp"].Inputs.NonIndexed().Pack(
		big.NewInt(0),
		big.NewInt(0),
	)
	if err != nil {
...
func TestDiluteBatch(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		expectedCallData, err := vouchercontract.PostageStampABI.Pack("increaseDepth", common.BytesToHash(batch.ID), newDepth)
		if err != nil {
			t.Fatal(err)
		}
...
func newDiluteEvent(postageContractAddress common.Address, batch *voucher.Batch) *types.Log {
	b, err := vouchercontract.PostageStampABI.Events["BatchDepthIncrease"].Inputs.NonIndexed().Pack(
		uint8(0),
		big.NewInt(0),
	)
	if err != nil {
